...
MENU_CONTACT = "🎁 ثبت نام و دریافت جایزه"
MENU_REMIX = "🎵 Remix Tm"
...
def start_contact_us(chat_id):
    ...
    send_message(
        chat_id,
        "🎁 <b>ثبت نام و دریافت جایزه</b>\n\n"
        "برای تکمیل ثبت‌نام، نیازمند اشتراک‌گذاری شماره تماست هستیم:",
...
def handle_contact_shared(chat_id, message):
    ...
    if ADMIN_CHAT_ID:
        ...
        print("نتیجه ارسال ثبت‌نام به ادمین:", result)
        if not result.get("ok"):
            print("❌ ارسال به ادمین ناموفق بود. ADMIN_CHAT_ID فعلی:", repr(ADMIN_CHAT_ID))
    else:
        print("⚠️ ADMIN_CHAT_ID تنظیم نشده؛ اطلاعات ثبت‌نام فقط همینجا لاگ شد.")

    reset_state(chat_id)
    send_main_menu(chat_id, "🎉 ثبت‌نامت با موفقیت انجام شد! به‌زودی جایزه‌ت اطلاع‌رسانی می‌شه.")
