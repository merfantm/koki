...
def start_contact_us(chat_id):
    ...
    send_message(
        ...
        "برای هماهنگی و ارسال اکانت، لطفاً شماره تماستو به اشتراک بذار:",
...
FONT_PATH_NAZANIN = os.path.join(os.path.dirname(__file__), "fonts", "BNazanin.ttf")
PETTY_CASH_FONT_SIZE = 14
# تبدیل سایز فونت واقعی (پوینت) به پیکسل متناسب با DPI تصویر A4 خروجی
PETTY_CASH_DPI = 200
PETTY_CASH_FONT_PX = int(PETTY_CASH_FONT_SIZE * PETTY_CASH_DPI / 72)

PETTY_CASH_FIELDS = [
    ("date", "📅 تاریخ (شمسی) رو وارد کن (مثلاً 1405/05/07):"),
    ("title", "📝 عنوان رو وارد کن:"),
    ("place", "📍 محل استفاده رو وارد کن:"),
    ("amount", "💰 مبلغ رو وارد کن:"),
    ("store", "🏪 نام فروشگاه رو وارد کن:"),
    ("address", "🗺 آدرس فروشگاه رو وارد کن:"),
]
...
def prepare_persian(text):
    """متن فارسی رو برای رسم صحیح (اتصال حروف + راست‌به‌چپ) آماده می‌کنه"""
...
def load_nazanin_font(size_px):
    if os.path.exists(FONT_PATH_NAZANIN):
        return ImageFont.truetype(FONT_PATH_NAZANIN, size_px)
    print("⚠️ فونت BNazanin.ttf پیدا نشد؛ از فونت پیش‌فرض (بدون پشتیبانی صحیح فارسی) استفاده می‌شه.")
    return ImageFont.load_default()


def start_petty_cash(chat_id):
    state = get_state(chat_id)
    state["mode"] = "petty_cash"
    state["petty_cash_data"] = {}
    state["petty_cash_step"] = 0
    send_message(
        chat_id,
        "💵 <b>ثبت تنخواه</b>\n\nاطلاعات رو مرحله به مرحله وارد کن.\n\n" + PETTY_CASH_FIELDS[0][1],
        keyboard=cancel_only_keyboard(),
    )


def handle_petty_cash_input(chat_id, text):
    state = get_state(chat_id)
    step = state["petty_cash_step"]
    key, _ = PETTY_CASH_FIELDS[step]
    state["petty_cash_data"][key] = text.strip()
    state["petty_cash_step"] += 1

    if state["petty_cash_step"] < len(PETTY_CASH_FIELDS):
        next_prompt = PETTY_CASH_FIELDS[state["petty_cash_step"]][1]
        send_message(chat_id, next_prompt, keyboard=cancel_only_keyboard())
    else:
        finish_petty_cash(chat_id)


def build_petty_cash_pdf(data, output_path):
    """ساخت یک صفحه A4 افقی با جدول اطلاعات تنخواه (متن فارسی درست‌شده)"""
    page_w = int(297 / 25.4 * PETTY_CASH_DPI)   # A4 افقی: عرض ۲۹۷میلی‌متر
    page_h = int(210 / 25.4 * PETTY_CASH_DPI)   # ارتفاع ۲۱۰میلی‌متر

    img = Image.new("RGB", (page_w, page_h), "white")
    draw = ImageDraw.Draw(img)

    title_font = load_nazanin_font(int(PETTY_CASH_FONT_PX * 1.6))
    cell_font = load_nazanin_font(PETTY_CASH_FONT_PX)

    margin = int(0.06 * page_w)
    table_top = int(0.22 * page_h)
    table_bottom = page_h - margin
    table_left = margin
    table_right = page_w - margin

    # عنوان فرم
    title_text = prepare_persian("فرم ثبت تنخواه")
    tw = draw.textlength(title_text, font=title_font)
    draw.text(((page_w - tw) / 2, int(0.08 * page_h)), title_text, fill="black", font=title_font)

    rows = [(PETTY_CASH_LABELS[k], data.get(k, "")) for k, _ in PETTY_CASH_FIELDS]
    row_h = (table_bottom - table_top) / len(rows)
    col_split = table_left + int((table_right - table_left) * 0.28)  # ستون برچسب سمت راست، چون RTL

    # جدول بیرونی
    draw.rectangle([table_left, table_top, table_right, table_bottom], outline="black", width=3)
    # خط جداکننده دو ستون
    draw.line([(col_split, table_top), (col_split, table_bottom)], fill="black", width=2)

    for i, (label, value) in enumerate(rows):
        y_top = table_top + i * row_h
        y_bottom = y_top + row_h
        if i > 0:
            draw.line([(table_left, y_top), (table_right, y_top)], fill="black", width=1)

        label_text = prepare_persian(label)
        value_text = prepare_persian(value)

        # برچسب: ستون سمت راست (چسبیده به لبه راست جدول)
        lw = draw.textlength(label_text, font=cell_font)
        draw.text((table_right - 20 - lw, y_top + (row_h - PETTY_CASH_FONT_PX) / 2), label_text, fill="black", font=cell_font)

        # مقدار: ستون سمت چپ (بین col_split و لبه چپ جدول)، چسبیده به لبه راست همون ستون
        vw = draw.textlength(value_text, font=cell_font)
        draw.text((col_split - 20 - vw, y_top + (row_h - PETTY_CASH_FONT_PX) / 2), value_text, fill="black", font=cell_font)

    img.save(output_path, "PDF", resolution=PETTY_CASH_DPI)


def build_petty_cash_excel(data, output_path):
    import openpyxl
    ...
    font_normal = Font(name="B Nazanin", size=14)
    font_title = Font(name="B Nazanin", size=18, bold=True)

    ws.merge_cells("A1:B1")
    title_cell = ws["A1"]
    title_cell.value = "فرم ثبت تنخواه"
    title_cell.font = font_title
    title_cell.alignment = Alignment(horizontal="center", vertical="center")
    ws.row_dimensions[1].height = 30

    for i, (key, _) in enumerate(PETTY_CASH_FIELDS, start=2):
        label_cell = ws.cell(row=i, column=1, value=PETTY_CASH_LABELS[key])
        value_cell = ws.cell(row=i, column=2, value=data.get(key, ""))
...
def finish_petty_cash(chat_id):
    state = get_state(chat_id)
    data = state["petty_cash_data"]

    send_chat_action(chat_id, "upload_document")
    send_message(chat_id, "⏳ در حال ساخت فایل‌های PDF و Excel...")

    pdf_path = os.path.join(TEMP_DIR, f"{chat_id}_petty_cash.pdf")
    xlsx_path = os.path.join(TEMP_DIR, f"{chat_id}_petty_cash.xlsx")

    try:
        build_petty_cash_pdf(data, pdf_path)
        send_document(chat_id, pdf_path, caption="✅ <b>فایل PDF تنخواه آماده است!</b>")
    except Exception as e:
        print("خطا در ساخت PDF تنخواه:", e)
        send_message(chat_id, "❌ مشکلی تو ساخت PDF پیش اومد.")

    try:
        build_petty_cash_excel(data, xlsx_path)
        send_document(chat_id, xlsx_path, caption="✅ <b>فایل Excel تنخواه آماده است!</b>")
    except Exception as e:
        print("خطا در ساخت اکسل تنخواه:", e)
        send_message(chat_id, "❌ مشکلی تو ساخت Excel پیش اومد.")
...
# ------------------ قابلیت: تبدیل دقیق ورد به PDF (با LibreOffice) ------------------

def start_word_to_pdf(chat_id):
    state = get_state(chat_id)
    state["mode"] = "word_to_pdf"
    send_message(
        chat_id,
        "📄 فایل ورد (.docx) رو برام بفرست تا دقیقاً همون شکلی (فونت، چیدمان، جدول‌ها) تبدیلش کنم به PDF:",
        keyboard=cancel_only_keyboard(),
    )


def handle_word_document(chat_id, message):
    document = message["document"]
    file_name = document.get("file_name", "") or "file.docx"

    if not file_name.lower().endswith(".docx"):
        send_message(chat_id, "❌ فقط فایل با پسوند .docx رو قبول می‌کنم.")
        return

    send_chat_action(chat_id, "upload_document")
    send_message(chat_id, "⏳ در حال تبدیل فایل به PDF... (ممکنه چند ثانیه طول بکشه)")

    input_path = os.path.join(TEMP_DIR, f"{chat_id}_input.docx")
    output_dir = os.path.join(TEMP_DIR, f"{chat_id}_lo_out")
    os.makedirs(output_dir, exist_ok=True)

    try:
        download_file(document["file_id"], input_path)
    except Exception as e:
        print("خطا در دانلود فایل ورد:", e)
        send_message(chat_id, "❌ نتونستم فایل رو دانلود کنم.")
        reset_state(chat_id)
        send_main_menu(chat_id)
        return

    profile_dir = f"/tmp/lo_profile_{chat_id}"
    cmd = [
    ...
    try:
        ...
        print("خروجی LibreOffice (stdout):", result.stdout)
        print("خروجی LibreOffice (stderr):", result.stderr)
    except Exception as e:
        print("خطا در اجرای LibreOffice:", e)
        send_message(chat_id, "❌ مشکلی تو تبدیل فایل پیش اومد.")
        reset_state(chat_id)
        send_main_menu(chat_id)
        return
    ...
    if os.path.exists(output_pdf):
        send_document(chat_id, output_pdf, caption="✅ <b>فایل PDF آماده است!</b>")
    else:
        send_message(chat_id, "❌ تبدیل انجام نشد. مطمئن شو فایل سالم و واقعاً .docx باشه.")

    for p in (input_path, output_pdf):
        if os.path.exists(p):
            os.remove(p)
...
def handle_message(message):
    ...
    if mode == "petty_cash" and text:
        handle_petty_cash_input(chat_id, text)
        return
