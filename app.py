...
# ------------------ قابلیت: ثبت تنخواه (چند ردیفی، خروجی Excel) ------------------

PETTY_CASH_ROW_FIELDS = [
    ("date", "📅 تاریخ (شمسی) رو وارد کن (مثلاً 1405/05/07):"),
    ("title", "📝 عنوان رو وارد کن:"),
    ("place", "📍 محل هزینه رو وارد کن:"),
    ("amount", "💰 مبلغ رو وارد کن (فقط عدد):"),
    ("vat", "➕ ارزش‌افزوده رو وارد کن (اگه نداره بنویس 0):"),
...
def to_english_digits(text):
    ...
    arabic = "٠١٢٣٤٥٦٧٨٩"
    table = {}
    for p, a, e in zip(persian, arabic, "0123456789"):
        table[p] = e
        table[a] = e
...
def parse_number(text):
    cleaned = to_english_digits(text).replace(",", "").replace("،", "").strip()
    try:
        num = float(cleaned)
...
def petty_cash_row_done_keyboard():
    return {
        "keyboard": [
            [{"text": "➕ ثبت هزینه بعدی"}],
            [{"text": DONE_BTN}],
...
def start_petty_cash(chat_id):
    state = get_state(chat_id)
    state["mode"] = "petty_cash_field"
    state["petty_cash_rows"] = []
    state["petty_cash_current"] = {}
    state["petty_cash_field_index"] = 0
    send_message(
        chat_id,
        "💵 <b>ثبت تنخواه</b>\n\nاطلاعات هزینه اول رو وارد کن.\n\n" + PETTY_CASH_ROW_FIELDS[0][1],
        keyboard=cancel_only_keyboard(),
    )


def handle_petty_cash_field_input(chat_id, text):
    state = get_state(chat_id)
    idx = state["petty_cash_field_index"]
    key, _ = PETTY_CASH_ROW_FIELDS[idx]
    state["petty_cash_current"][key] = text.strip()
    idx += 1
    state["petty_cash_field_index"] = idx

    if idx < len(PETTY_CASH_ROW_FIELDS):
        send_message(chat_id, PETTY_CASH_ROW_FIELDS[idx][1], keyboard=cancel_only_keyboard())
    else:
        finalize_petty_cash_row(chat_id)


def finalize_petty_cash_row(chat_id):
    state = get_state(chat_id)
    data = state["petty_cash_current"]

    amount = parse_number(data.get("amount", "0"))
    vat = parse_number(data.get("vat", "0"))
    total = amount + vat

    data["amount"] = format_number(amount)
    data["vat"] = format_number(vat)
    data["total"] = format_number(total)
    data["row_no"] = len(state["petty_cash_rows"]) + 1

    state["petty_cash_rows"].append(data)
    state["petty_cash_current"] = {}
    state["petty_cash_field_index"] = 0
    state["mode"] = "petty_cash_row_done"

    send_message(
        chat_id,
        f"✅ هزینه شماره {data['row_no']} ثبت شد.\nهزینه‌ی بعدی رو هم می‌خوای وارد کنی؟",
        keyboard=petty_cash_row_done_keyboard(),
    )


def build_petty_cash_excel(rows, output_path):
    import openpyxl
    ...
    font_header = Font(name="B Nazanin", size=14, bold=True)
    font_title = Font(name="B Nazanin", size=18, bold=True)

    n_cols = len(PETTY_CASH_COLUMNS)

    # ردیف عنوان کلی
    ws.merge_cells(start_row=1, start_column=1, end_row=1, end_column=n_cols)
    title_cell = ws.cell(row=1, column=1, value="لیست هزینه تنخواه")
    title_cell.font = font_title
    title_cell.alignment = Alignment(horizontal="center", vertical="center")
    ws.row_dimensions[1].height = 30

    # ردیف سربرگ ستون‌ها
    for col_idx, (key, label) in enumerate(PETTY_CASH_COLUMNS, start=1):
    ...
    for r, row_data in enumerate(rows, start=3):
        for col_idx, (key, _) in enumerate(PETTY_CASH_COLUMNS, start=1):
            cell = ws.cell(row=r, column=col_idx, value=row_data.get(key, ""))
            cell.font = font_normal
            cell.alignment = Alignment(horizontal="center", vertical="center")
            cell.border = border
        ws.row_dimensions[r].height = 22

    # عرض ستون‌ها متناسب با نوع محتوا
    widths = {
        "row_no": 8, "date": 14, "title": 20, "place": 16,
        "amount": 14, "vat": 14, "total": 14, "store": 16, "address": 26,
    }
    for col_idx, (key, _) in enumerate(PETTY_CASH_COLUMNS, start=1):
        col_letter = ws.cell(row=2, column=col_idx).column_letter
        ws.column_dimensions[col_letter].width = widths.get(key, 16)
...
def finish_petty_cash(chat_id):
    state = get_state(chat_id)
    rows = state.get("petty_cash_rows", [])

    if not rows:
        send_message(chat_id, "⚠️ هنوز هیچ هزینه‌ای ثبت نکردی!")
        return

    send_chat_action(chat_id, "upload_document")
    send_message(chat_id, f"⏳ در حال ساخت فایل Excel از {len(rows)} هزینه...")

    xlsx_path = os.path.join(TEMP_DIR, f"{chat_id}_petty_cash.xlsx")

    try:
        build_petty_cash_excel(rows, xlsx_path)
...
def handle_message(message):
    ...
    if text == "➕ ثبت هزینه بعدی" and mode == "petty_cash_row_done":
        state["mode"] = "petty_cash_field"
        send_message(chat_id, PETTY_CASH_ROW_FIELDS[0][1], keyboard=cancel_only_keyboard())
        return
    ...
    if mode == "petty_cash_field" and text:
        handle_petty_cash_field_input(chat_id, text)
        return
