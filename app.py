...
MENU_REMIX = "🎵 Remix Tm"
MENU_INSTAGRAM = "📷 دانلود پروفایل اینستاگرام"
...
# ------------------ قابلیت: دانلود پروفایل اینستاگرام ------------------
...
def get_instagram_profile_pic_url(username):
    ...
    if resp.status_code != 200:
        print("خطا در دریافت صفحه اینستاگرام:", resp.status_code)
        return None
...
def start_instagram_download(chat_id):
    state = get_state(chat_id)
    state["mode"] = "instagram_download"
    send_message(
        chat_id,
        "📷 لینک یا یوزرنیم پروفایل اینستاگرام رو بفرست (فقط پروفایل‌های عمومی):",
        keyboard=cancel_only_keyboard(),
    )


def handle_instagram_link(chat_id, text):
    username = extract_instagram_username(text)
    if not username:
        send_message(chat_id, "❌ نتونستم یوزرنیم رو تشخیص بدم. دوباره امتحان کن.")
        return

    send_chat_action(chat_id, "upload_photo")
    send_message(chat_id, f"⏳ در حال گرفتن عکس پروفایل @{username}...")

    try:
        pic_url = get_instagram_profile_pic_url(username)
    except Exception as e:
        print("خطا در ارتباط با اینستاگرام:", e)
        pic_url = None

    if not pic_url:
        send_message(
            chat_id,
            "❌ نتونستم عکس پروفایل رو پیدا کنم.\n"
            "ممکنه یوزرنیم اشتباه باشه، پروفایل خصوصی باشه، یا اینستاگرام درخواست رو مسدود کرده باشه.",
        )
        reset_state(chat_id)
        send_main_menu(chat_id)
        return
    ...
    try:
        img_resp = requests.get(pic_url, timeout=20)
        temp_path = os.path.join(TEMP_DIR, f"{chat_id}_ig_profile.jpg")
        with open(temp_path, "wb") as f:
            f.write(img_resp.content)
        send_document(chat_id, temp_path, caption=f"✅ عکس پروفایل @{username}")
        os.remove(temp_path)
    except Exception as e:
        print("خطا در دانلود عکس پروفایل:", e)
        send_message(chat_id, "❌ مشکلی تو دانلود عکس پیش اومد.")
...
def handle_message(message):
    ...
    if mode is None:
        ...
        if text == MENU_INSTAGRAM:
            start_instagram_download(chat_id); return
    ...
    if mode == "instagram_download" and text:
        handle_instagram_link(chat_id, text)
        return
